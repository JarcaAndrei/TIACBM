...
import bisect
import math
#Identical code with CB-NTR, just adding TIACBM
source_dir = './'
prefix = 'reuters' 
suffix = 'rand123'
...
def decade_to_words(decade_str):
    decade = int(decade_str[:4])  # Convert "1980s" → 1980
    decade_short = int(decade_str[2:3])  # Extract "80" from "1980s"
    
    full_text = f"the {num2words(decade)}s"  # "the nineteen eighties"
    #short_text = f"the {num2words(decade_short)}ies"  # "the eighties"
    #apostrophe_text = f"the '{decade_short}0s"  # "the '80s"
...
for entry in data_train:
    processed_text = preprocess_text1(entry["text"]) 
    ...
    def __init__(self, documents, mask_prob, model, sentiment_dict=None):
        ...
        if self.mask_prob != 0:
            for i in tqdm(range(len(self.encodings)), desc="Masking tokens"):
                self.encodings[i]['input_ids'] = self.mask_tokens_attention_based(self.encodings[i]['input_ids'], self.encodings[i]['attention_mask'], self.docs[i])
    def __len__(self):
        return len(self.documents)
...
for epoch in trange(epochs, desc="Epoch"):
    ...
    log_dir = os.path.join(source_dir, 'logs/setforlife')
