...
class CustomDataset_train(Dataset):
    ...
    def mask_tokens(self, input_ids):
        ...
        for word in words:
            ...
            if synset:
                ...
                if pos_id in self.sentiment_dict:
                    ...
                    word_indices[word] = get_word_token_indices(tokens, word, tokenizer)
                else:
        ...
        num_to_mask = max(1, int(total_words * self.mask_prob * 3))
            ...
            for indices in word_indices[word]:
                for idx in indices:
                    ...
                    if random.random() <=  (2 - word_objectivity_scores[word]) * self.mask_prob:
                        if masked_tokens[idx] not in tokenizer.all_special_tokens:  # Exclude special tokens
