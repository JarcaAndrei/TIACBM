...
DATA_PATH = "path/to/pan19problem00001"
BATCH_SIZE = 8
EPOCHS = 30
LR = 1e-4
MAX_LEN = 512
...
model = BertForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=len(candidate_authors)).to('cuda')

criterion = nn.CrossEntropyLoss()
optimizer = optim.AdamW(model.parameters(), lr=LR)
probs = [0.35,0.3,0.25] 
probs = [0,0,0] 
probs = np.linspace(0.3,0,30)
#0.4 0.25 0.1 
...
#best curr 0, 35 0 0.25
def train():
    model.train()
    probs = [0,0,0]  
    #probs = [0.3,0,0.25]
    data1 = DataLoader(AuthorshipDataset(DATA_PATH, probs[0], tokenizer, MAX_LEN, model), batch_size=BATCH_SIZE, shuffle=True)
    #0.79
    #821
    #probs = [0.35,0,0.25]
    #probs = [0.35,0.,0.25]
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch==1:
            ...
            #probs = [0.15,0,0.1]  
            #probs = [0.35,0.35,0.25] 
            probs = [0.35,0.3,0.25]  
            data1 = DataLoader(AuthorshipDataset(DATA_PATH, probs[0], tokenizer, MAX_LEN, model), batch_size=BATCH_SIZE, shuffle=True)
            data2 = DataLoader(AuthorshipDataset(DATA_PATH, probs[1], tokenizer, MAX_LEN, model), batch_size=BATCH_SIZE, shuffle=True)
            data3 = DataLoader(AuthorshipDataset(DATA_PATH, probs[2], tokenizer, MAX_LEN, model), batch_size=BATCH_SIZE, shuffle=True)
        
            #probs = [0.3,0,0.25]  
        #train_loader = DataLoader(AuthorshipDataset(DATA_PATH, probs[epoch], tokenizer, MAX_LEN, model), batch_size=BATCH_SIZE, shuffle=True)
